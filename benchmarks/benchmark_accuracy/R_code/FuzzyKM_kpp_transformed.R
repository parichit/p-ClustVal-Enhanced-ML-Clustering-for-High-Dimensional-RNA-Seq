...
for(i in 1:length(file_list)){
  ...
  if (data_list[i] == "Pollen"){
    ...
    temp <- c(data_name, "T-FZKM", num_clusters, avg_ari_raw, ari_std, avg_nmi_raw, nmi_std) 
...
write.table(all_output, "FuzzyKM_kpp_all_results_transformed.csv", sep = ",", row.names = FALSE)
write.table(avg_output, "FuzzyKM_kpp_avg_results_transformed.csv", sep = ",", row.names = FALSE)
