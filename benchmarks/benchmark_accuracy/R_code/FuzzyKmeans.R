...
# file_list <- c("pollen_raw.csv", "darmanis_raw.csv")
...
for(i in 1:length(file_list)){
    ...
    if (data_list[i] == "Pollen"){
      ...
      temp <- c(data_name, "FZKM", num_clusters, avg_ari, ari_std, avg_nmi, nmi_std) 
...
write.table(all_output, "FuzzyKM_rand_all_runs.csv", sep = ",", row.names = FALSE)
write.table(avg_output, "FuzzyKM_rand_avg_runs.csv", sep = ",", row.names = FALSE)


print(paste("Completed execution on:", Sys.time()))

# filepath = file.path(BASE_PATH, "BreastCancer.csv")
# data <- read.csv2(filepath, sep=",", stringsAsFactors=FALSE)
# labels = read.csv2(file.path(BASE_PATH, "labels_BreastCancer.csv"), stringsAsFactors = FALSE)
# 
# results <- cmeans(data, centers = data[c(200, 449, 515), ], iter.max = 2000, method = "cmeans", m = 1.5)
# adjustedRandIndex(labels[, 1], results$cluster)
# NMI(labels[, 1], results$cluster)

# for (k in 0:rep){
#   print(k)
# }

# mean(as.numeric(all_output[(all_output[, 2] == "BreastCancer" & all_output[, 3] == 3), 5]))
