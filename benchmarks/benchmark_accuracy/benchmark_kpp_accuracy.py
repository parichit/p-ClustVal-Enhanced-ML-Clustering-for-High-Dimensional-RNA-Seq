...
# file_list = ["darmanis_raw.csv", "pollen_raw.csv"]
# data_list = ["Darmanis", "Pollen"]
# label_list = ["labels_darmanis.csv", "labels_pollen.csv"]
# centroids_file = ["DarmanisCentroids", "PollenCentroids"]
...
for i in range(len(file_list)):
    ...
    for rep in range(num_rep):
        ...
        # kmeans = KMeans(n_clusters=num_clusters, random_state=seeds[i, rep], init="k-means++", n_init=5).fit(data)
        kmeans = KMeans(n_clusters=num_clusters, init=data.iloc[centroids_indices,], n_init=5).fit(data)
        
        temp_raw_ari.append(ari(kmeans.labels_, labels))
        temp_raw_nmi.append(nmi(kmeans.labels_, labels))
        temp_alg.append("KM")
    ...
    for rep in range(num_rep):
        ...
        # kmeans_std = KMeans(n_clusters=num_clusters, random_state=seeds[i, rep], init="k-means++", n_init=5).fit(X_std)
        kmeans_std = KMeans(n_clusters=num_clusters, init=X_std[centroids_indices], n_init=5).fit(X_std)
        
        temp_std_ari.append(ari(kmeans_std.labels_, labels))
        temp_std_nmi.append(nmi(kmeans_std.labels_, labels))
        temp_alg.append("S-KM")
    ...
    for rep in range(num_rep):
        ...
        # kmeans_transformed = KMeans(n_clusters=num_clusters, random_state=seeds[i, rep], init="k-means++", n_init=5).fit(new_data)
        kmeans_transformed = KMeans(n_clusters=num_clusters, init=new_data.iloc[centroids_indices], n_init=5).fit(new_data)
        
        temp_transformed_ari.append(ari(kmeans_transformed.labels_, labels))
        temp_transformed_nmi.append(nmi(kmeans_transformed.labels_, labels))
        temp_alg.append("T-KM")
    ...
    temp = pd.DataFrame(list(zip(temp_data, temp_alg, temp_cluster, 
                                 temp_raw_ari+temp_std_ari+
                                 temp_raw_ari_ac+temp_raw_ari_em+
                                 temp_transformed_ari,
                                 
                                 temp_raw_nmi+temp_std_nmi+
                                 temp_raw_nmi_ac+temp_raw_nmi_em+
                                 temp_transformed_nmi)), 
...
avg_results.to_csv("avg_results_kpp_accuracy.csv", index=False)
all_results.to_csv("all_results_kpp_accuracy.csv", index=False)
