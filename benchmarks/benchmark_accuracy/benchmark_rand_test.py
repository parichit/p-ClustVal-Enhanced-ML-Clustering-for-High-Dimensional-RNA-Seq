...
for i in range(len(file_list)):
    ...
    # Expectation Maximization on transformed data  
    for rep in range(num_rep):
        
        raw_centroids = load_centroids(centroids_data, data_list[i], rep)
        
        raw_labels = GaussianMixture(n_components=num_clusters, covariance_type="diag", 
                                     means_init=data.iloc[raw_centroids], n_init=5).fit_predict(new_data)
...
avg_results.to_csv("avg_results_random_accuracy_algorithms.csv", index=False)
all_results.to_csv("all_results_random_accuracy_algorithms.csv", index=False)
