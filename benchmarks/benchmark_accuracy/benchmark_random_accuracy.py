...
centroids_data = pd.read_csv("random_centroid_indices.csv", header=0)
...
for i in range(len(file_list)):
    ...
    for rep in range(num_rep):
        ...
        raw_centroids = load_centroids(centroids_data, data_list[i], rep)
        
        kmeans = KMeans(n_clusters=num_clusters, init=data.iloc[raw_centroids], n_init=5).fit(data)
    ...
    for rep in range(num_rep):
        ...
        std_centroids = load_centroids(centroids_data, data_list[i], rep)
        
        kmeans_std = KMeans(n_clusters=num_clusters, init=X_std[std_centroids], n_init=5).fit(X_std)
    ...
    for rep in range(num_rep):
        ...
        raw_labels = GaussianMixture(n_components=num_clusters, covariance_type="diag", 
                                     means_init=data.iloc[raw_centroids], n_init=5).fit_predict(data)
    ...
    for rep in range(num_rep):
        ...
        transformed_centroids = load_centroids(centroids_data, data_list[i], rep)
        
        kmeans_transformed = KMeans(n_clusters=num_clusters, init=new_data.iloc[transformed_centroids], n_init=5).fit(new_data)
...
avg_results = pd.DataFrame(list(zip(out_data, out_alg, out_clusters, out_ari, out_ari_std, 
                                    out_nmi, out_nmi_std)), 
                           columns=["Data", "Algorithm", "Clusters", "ARI", 
                                    "ARI_STD", "NMI", "NMI_STD"])


avg_results.to_csv("avg_results_random_accuracy.csv", index=False)
all_results.to_csv("all_results_random_accuracy.csv", index=False)
