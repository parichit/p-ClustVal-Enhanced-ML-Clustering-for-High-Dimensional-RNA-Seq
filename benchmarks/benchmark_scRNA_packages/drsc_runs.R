...
data_list <- c("Pollen", "Darmanis", "Mouse_pan", "Muraro", "QSLimb", "QSTrachea", "QSLung", 
...
# file_list <- c("darmanis_raw.csv", "mouse_pan.csv")
# data_list <- c("Darmanis", "Mouse_pan")
# labels_list <- c("labels_darmanis.csv", "labels_mouse_pan.csv")
...
for(i in 1:length(file_list)){
  ...
  if (data_list[i] == "Pollen"){
    ...
    # Seurat specific preprocessing
    Seurat_data <- CreateSeuratObject(counts = raw_data)
    ...
    # Run DR.SC
    Seurat_data <- DR.SC(Seurat_data, K=num_clusters, q=15, platform= "scRNAseq")
    ...
    # Append the output
    temp <- c(data_name, "DRSC", num_clusters, length(unique(Idents(Seurat_data))), ari, nmi)
    ...
    # Seurat specific preprocessing
    transformed_Seurat_data <- CreateSeuratObject(counts = transformed_data)

    transformed_Seurat_data <- NormalizeData(transformed_Seurat_data, normalization.method = "LogNormalize", scale.factor = 10000)
    transformed_Seurat_data <- FindVariableFeatures(transformed_Seurat_data, selection.method = "vst")


    # Run DR.SC
    transformed_Seurat_data <- DR.SC(transformed_Seurat_data, K=num_clusters, q=15, platform="scRNAseq")


    # Calculate the statistics
    ari <- round(adjustedRandIndex(labels[, 1], Idents(transformed_Seurat_data)), 2)
    nmi <- round(NMI(labels[, 1], Idents(transformed_Seurat_data)), 2)
    

    # Append the output
    temp <- c(data_name, "T-DRSC", num_clusters, length(unique(Idents(Seurat_data))), ari, nmi)
    final_output <- rbind(final_output, temp)
...
write.table(final_output, "DRSC_results.csv", sep = ",", row.names = FALSE)