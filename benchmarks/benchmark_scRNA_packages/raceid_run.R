...
file_list <- c("pollen_raw.csv", "darmanis_raw.csv", "mouse_pan.csv", 
            "Muraro_raw.csv", "QSLimb_raw.csv", "QSTrachea_raw.csv", 
            "QSLung_raw.csv", "QSDiaphragm_raw.csv", 
             "Q10XSpleen_raw.csv")

data_list <- c("Pollen", "Darmanis", "Mouse_pan", "Muraro", "QSLimb", 
"QSTrachea", "QSLung", 
             "QSDiaphragm", "Q10XSpleen")

labels_list <- c("labels_pollen.csv", "labels_darmanis.csv", 
              "labels_mouse_pan.csv", "labels_Muraro.csv", "labels_QSLimb.csv", 
...
for(i in 1:length(file_list)){
  ...
  if (data_list[i] == "Pollen"){
    ...
    # Choose the size of random sample
    rand_samp <- datascale(ncol(raw_data))
    sc <- SCseq(raw_data)
    ...
    # Run DR.SC
    sc <- clustexp(sc, cln=num_clusters, sat=FALSE, 
    bootnr=rand_samp, FUNcluster = "kmeans")

    # Calculate the statistics
    ari <- round(adjustedRandIndex(labels, as.numeric(sc@cluster$kpart)), 2)
    nmi <- round(NMI(labels, as.numeric(sc@cluster$kpart)), 2)


    # Append the output
    temp <- c(data_name, "RaceID", num_clusters, 
    length(unique(as.numeric(sc@cluster$kpart))), ari, nmi)
    ...
    sc <- SCseq(transformed_data)
    sc <- filterdata(sc, mintotal=100)
        
    sc <- compdist(sc, metric="pearson")

    # Run RaceID
    sc <- clustexp(sc, cln=num_clusters, sat=FALSE, bootnr=rand_samp, 
    FUNcluster = "kmeans")
    ...
    # Append the output
    temp <- c(data_name, "T-RaceID", num_clusters, 
    length(unique(as.numeric(sc@cluster$kpart))), 
    ari, nmi)
...
write.table(final_output, "RaceID_results.csv", sep = ",", row.names = FALSE)